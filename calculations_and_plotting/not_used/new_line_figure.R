...
#' @return figure
#' @export
line_figure <- function(data_table,merge_variable,value_unit,title_name,character_list=NULL,x_label="Year",lower_limit=0,upper_limit=NA,return_static=TRUE,source_citation=NULL,modifications=NULL,subtitle_description=NULL,future_date=NULL){
  ...
  category_count <- length(unique(data_table$variable))
  ...
  if(is.null(future_date)){
    figure <- ggplot(data_table, aes(x=x_unit,y=value,color=variable)) +
    ...
    figure <- ggplot() +
      geom_line(data=data_table[x_unit<future_date],mapping=aes(x=x_unit,y=value,color=variable,group=variable,text=paste0(x_label,": ",x_unit,"\n","Value: ",round(value,4),"\n","Variable: ",variable))) +
